...
public:
  ...
  bool set(const char *length, const char *dec);
  uint32 length() const
  {
    return m_length;
  }
  ...
  bool has_explicit_length() const { return m_has_explicit_length; }
  bool has_explicit_dec() const { return m_has_explicit_dec; }
};
...
public:
  ...
  {
    m_handler= handler;
    m_charset= coll.charset_collation();
    Lex_length_and_dec_st::operator=(length_and_dec);
  ...
  Lex_charset_collation lex_collation() const
  {
    return Lex_charset_collation(m_charset,
